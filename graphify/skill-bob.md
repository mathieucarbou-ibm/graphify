...
if ! "$PYTHON" -c "import graphify" 2>&1 | grep -v "__version__" >/dev/null; then
...
**Before starting:** note whether `--mode deep` was given. You must pass `DEEP_MODE=true` to extraction if it was. Track this from the original invocation - do not lose it.
...
> **No other API keys are read.** When `GEMINI_API_KEY`/`GOOGLE_API_KEY` are unset, semantic extraction falls to the host agent itself — the running session is the LLM. For Bob, use the headless extraction approach since Bob doesn't have the same multi-agent capabilities as Claude: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, use the Gemini backend if available or extract inline.

**Run Part A (AST) and Part B (semantic) in parallel if possible. Both can run simultaneously since they operate on different file types. Merge results in Part C.**

Note: Parallelizing AST + semantic saves 5-15s on large corpora. AST is deterministic and fast; start it while semantic extraction is processing docs/papers.
...
**Fast path:** If detection found zero docs, papers, and images (code-only corpus), skip Part B entirely and go straight to Part C. AST handles code - there is nothing for semantic extraction to do. **First write an empty semantic file** so Part C's merge has its input (it reads `.graphify_semantic.json` unconditionally; without this a code-only run hits `FileNotFoundError`):
...
Before extracting any files, check which files already have cached extraction results:
...
**Step B1 - Extract uncached files**

For Bob, we'll use the headless extraction approach since Bob doesn't have the same multi-agent capabilities as Claude:
...
if not uncached_file.exists() or not uncached_file.read_text(encoding="utf-8").strip():
    Path("graphify-out/.graphify_semantic_new.json").write_text(json.dumps({"nodes":[],"edges":[],"hyperedges":[],"input_tokens":0,"output_tokens":0}, ensure_ascii=False), encoding="utf-8")
    print("All files cached - skipping semantic extraction")
else:
    files = [Path(f.strip()) for f in uncached_file.read_text(encoding="utf-8").split("\n") if f.strip()]
    print(f"Extracting {len(files)} uncached files...")
    
    # Use Gemini if available, otherwise will use default backend
...
new = json.loads(Path("graphify-out/.graphify_semantic_new.json").read_text(encoding="utf-8")) if Path("graphify-out/.graphify_semantic_new.json").exists() else {"nodes":[],"edges":[],"hyperedges":[],"input_tokens":0,"output_tokens":0}

merged = {
    "nodes": cached.get("nodes", []) + new.get("nodes", []),
